...
if (param('formName') eq "login") 
  {
    ...
    $statement = "select password, ler_agreement from CereonUser where username='$username'";
    ...
    # if user has right password and the field ler_agreement is true, show the data
	...
	{ &showData($username, crypt $password, "ncgr" ); }
      ...
      # if user has right password and the field ler_agreement is not true, show the registration form
      else
