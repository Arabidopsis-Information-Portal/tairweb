...
# added to put in new header
require "../../format_tairNew.pl";

my $rootDir = $ENV{DOCUMENT_ROOT} . "/..";
my $dataDir = "$rootDir/data/motiffinder/";
my $seq_datadir = "$rootDir/data/sequences/" ;
my $seq_indexfiledir = "$rootDir/data/sequences/" ;
my $UPSTREAM_500=$seq_datadir."At_upstream_500";
my $UPSTREAM_1000=$seq_datadir."At_upstream_1000";
my $UPSTREAM_3000=$seq_datadir."At_upstream_3000";
my $cgi = new CGI;
...
print $cgi->header();	
#print "$upstream $seq_datasetpath $seq_indexfile\n";
...
print $cgi->header();	
    ...
    my $tmpFile = "$rootDir/tmp/oligoAnalysis_"; 
    ...
    $nodeSize = oligoAnalysisTools::countSeqs('file'=>$tmpFile);
    ($nodeCounts , $nodeCountsNorm) = oligoAnalysisTools::countOligos('file'=>$tmpFile, 
                                       'oligoLength'=>$oligoLength, 
                                       'bothStrands'=>$CHECK_BOTH_STRANDS);
    $pdlHypNorm = oligoAnalysisTools::calcBinomial( 'bkgrndCounts'=>$bkgrndCountsNorm, 
...
sub start_page{
    tair_header("Motif Analysis in promoter/upstream sequences");
