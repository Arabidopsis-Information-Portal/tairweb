...
my $slim_ann_file = "$ENV{DOCUMENT_ROOT}/../data/TAIR_PO";
...
my $tmp_params = "$ENV{DOCUMENT_ROOT}/../tmp/getpo/$sessID";
if($lociStr){
	print "Set-Cookie: getpoid=$sessID; path=/\n";
...
sub output_html {
    ...
    tair_header("PO Annotation");
    
#   print "<center>\n";
    

    print "<span class=mainheader>PO Annotations</span>\n";

    
    print "<TABLE border='1' width='100%' bgcolor='#ffffff'>\n";
    print "<TR><TD>Locus</TD><TD>Gene Model(s)</TD><TD>PO term <br /><font size=1>(links to Tair Keyword Browser)</font><br />(PO ID)</TD><TD>cat</TD><TD>code</TD><TD>Reference</TD><TD>Made by:<br />date last modified</TD></TR>\n";
    my $locus;
    foreach $locus (@loci) {
        my $locuslink;
	    ...
	    for (my $i=0; $i < scalar(@processed_infor); $i++) {
                my( $tair_id, $genemodel,$goterm, $goid, $term_id, $cat,$evicode, $ref,$by, $date) = split (/\t/, $processed_infor[$i]);
		...
		#	my $gobrowserlink = "<a href=http://godatabase.org/cgi-bin/go.cgi?query=$goid&view=details&search_constraint=terms&depth=0 target=\"_blank\">$goterm</a>";
		...
		if ( $i == 0) {
		    ...
		    print ">";
                ...
                print "$genemodel</TD><TD><A href=$browserlink target=_blank>$goterm</A><br />( $goid ) </TD><TD>$cat</TD><TD>$evidencelink</TD><TD>$analysisreflink</TD><TD>$institutionlink<br />$date";
		print "</TD></TR>\n";
	    }
	}
    }
...
sub output_error {
    ...
    tair_header("TAIR: PO Search Error");
    ...
    print "<a href=\"/tools/bulk/go/\">PO Search Page</a><BR><BR><BR>";
    print "</TABLE>\n";
...
sub  mergeGoSlim{
   ...
   while ( my ($key, $value) = each(%temp_hash) ) {
       ...
       push   @return_array , join ( "\t", $tair_id, $genemodel,$goterm, $goid, $term_id, $cat, $evicode, $ref, $by,$date);
   } 
   return @return_array ;
 }
