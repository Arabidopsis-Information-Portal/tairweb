...
if ( (ns4) || (ie4) || (ns6)) {
	...
	// only need to handle the mouse event when overlib() is called. by gc
...
function overlib350() {
	...
	// Show layer
	// disp(o3_status);
	// by gc, commentted out to 
	// allow the mouseMove event handler to display or reposition the layer. Because
	// that is more responsitive to mouse position.

	// Stickies should stay where they are.	
	if (o3_sticky) {

		// o3_allowmove = 0; by gc
...
function disp(statustext) {
	if ( (ns4) || (ie4) || (ns6) ) {
		// for sticky, this is always true
...
function placeLayer() {
	...
	if (o3_fixx > -1) {
		...
		if (o3_hpos == CENTER) { // Center
			...
			if (placeX < winoffset) placeX = winoffset;  // Added REB
		}
...
// Event handler, in case of not sticky, move the tooltips, modified by gc
...
function mouseMove(e) {
	...
	if (o3_showingsticky == 1){

		//Show layer, at this time, we already figure out mouse's position
		//So it is more responsitive to user's mouse move
		disp(o3_status);

	
		//alert("mouse moved");
		// make no sense to continue to track the mouse move event here	
		if (ns4) document.releaseEvents(Event.MOUSEMOVE);
               	document.onmousemove = null;
	} else {
