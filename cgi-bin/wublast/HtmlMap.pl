...
#
# Modified by Guanghong Chen April, 2002
# Turn this into a libray, so the cgi script can generally
# call generate_html_map_output() function, instead of a 
# system call, which will spawn a seperate process.
#
...
use lib "JCES-BLASTGRAPHIC/lib";
use lib "JCES-BLASTGRAPHIC";
...
my( $resultHelp ) = "/Blast/Blastresults.html";
my( $mapName ) = "imap";
my( $formFieldWidth ) = 80;         #edited by bengt, was 100
...
my( $DEBUG) = 0;
 ...
 #
 #
sub generate_html_map_output {

  my( $bfname, $showNamesP ) = @_;
  ...
  my( $horizRatio, $hit, $wrap, $wrapList, $parts );
  ...
  drawQuery( $img, $srcLength, $horizRatio, $black );
  
  drawGraph( $img, $parts, $showNamesP, $realWidth, $horizRatio, $black, $white, $bgColor3, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark );
  
  drawKey( $img, $realHeight, $grayDark, $black, $white, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark );
  
  drawStamp( $img, $realWidth, $realHeight, $grayDark, $black );
...
sub drawGraph
{
  my( $img, $parts, $showNamesP, $realWidth, $horizRatio, $black, $white, $bgColor3, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark) = @_;
  if ($DEBUG){
    dmsg( "drawGraph()...$kNumberOfPartitions" );
  }
  ...
  for( $pIndex = 0; $pIndex < $kNumberOfPartitions; $pIndex++ )
    {
      $part = $parts->getPartitionAt( $pIndex );

      if ($DEBUG){
	dmsg( "drawGraph(): partition \#$pIndex count =", $part->getCount() );
      }
	...
	{
	  $curBgColor = ( $curBgColor == $white ) ? $bgColor3 : $white;

	  $hspPos = drawWrap($img, $wrap, $parts, $showNamesP, $realWidth, $hspPos, $black, $white, $horizRatio, $curBgColor, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark);
	  	  
	  if ($DEBUG){
	    dmsg( "drawGraph(): hspPos \#$hspPos");
	  }	
      ...
      $countsRef = $parts->getPartitionElementsCountsRefAt( $pIndex );
	  ...
	  if( $ref->{ $kAllShowingP } == 0 )
	    {
	      ...
	      drawString($img, $countsStr, GD::gdSmallFont(),
			 $realWidth - $imgRightBorder + 3, $hspMid,
			 pickColorN( $pIndex, 0, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark  ), $black );
	    }
	}
    }
...
sub drawWrap
  {
    my( $img,  $wrap, $parts, $showNamesP, $realWidth, $hspPos, $black, $white, $horizRatio, $curBgColor, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark) = @_;
    ...
    $colorN = getColorNFromP( $wrap, 0, $parts );
    
    $hspBefore = $hspPos;
    if( $fwdCount > 0 )
    {
	#dmsg( "drawWrap(): fwd..." );
	$hspPos = drawDirection($img, $fwdRef->getBucketList(), $hspPos, 'plus', $white, $colorN , $horizRatio, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark);
	#dmsg( "drawWrap(): ...fwd", $hspPos );
    }
    if( $revCount > 0 )
    {
	#dmsg( "drawWrap(): rev..." );
	$hspPos = drawDirection($img, $revRef->getBucketList(), $hspPos, 'minus', $white, $colorN,  $horizRatio, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark );
	...
	$hspMid = getHspMid( $hspBefore, $hspAfter );
	#print STDERR "NOTE: $note, $x, $hspMid,$black\n";
...
sub drawDirection
  {
    my($img, $bucketList, $hspPos, $dir, $white, $colorN,  $horizRatio, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark ) = @_;
    my( $bucket );
    my( $regionList );
    my( $region );
	...
	while( $region = $regionList->shift() )
	  { 
	    ...
	    drawArrowedOutlinedFromN($img, $x1, $y1, $scaledLength, $dir, $white,$colorN, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark );
	  }
...
sub drawQuery
  {
    ...
    for( $rawX = $rawStep; $rawX < $length; $rawX += $rawStep )
      {
	$str = "$rawX";
	drawTick( $img, $str, $rawX, $ratio, $black );
      }
...
sub drawString
  {
    my( $img, $str, $font, $xpos, $ypos, $color, $black ) = @_;
    my( $end );
...
#   print "ONMOUSEOVER='document.daform.notes.value=\"$scoreDesc $englishDesc\"'";
    ...
    print "ONMOUSEOVER='document.daform.notes.value=\"$englishDesc\"'";
    print ">\n";
}
...
sub drawKey
{
  ...
  my ($img, $realHeight, $grayDark, $black, $white, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark  ) = @_;
  ...
  $strOffset = drawString( $img, "Fwd:", GD::gdMediumBoldFont(), $xpos, $ypos+1, $grayDark ,$black);
  $xpos += $strOffset + 4;
  drawArrowedOutlined($img, $xpos, int($ypos+$fontHeight/2), 9, 'plus', $grayDark, $grayDark, $white );
  
  $xpos += 18;
  drawString( $img, "Rev:", GD::gdMediumBoldFont(), $xpos, $ypos+1, $grayDark, $black );
  $xpos += $strOffset + 4;
  drawArrowedOutlined($img, $xpos, int($ypos+$fontHeight/2), 9, 'minus', $grayDark, $grayDark, $white );
  ...
  for( $dex = 0; $dex < 5; $dex++ )
    {
      $str = $barParts[ $dex*2 ];
      $clr = pickColorN( $barParts[ $dex*2 + 1 ], 0, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark  );
...
{
    my($img, $x1, $y1, $scaledLength, $dir, $white, $colorN, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark ) = @_;
    my( $light, $dark );

    $light = pickColorN( $colorN, 0, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark );
    $dark = pickColorN( $colorN, 1, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark  );

    drawArrowedOutlined($img, $x1, $y1, $scaledLength, $dir, $light, $dark, $white );
}

#----------------------------------------
sub drawArrowedOutlined
{
    my( $img, $x1, $y1, $scaledLength, $dir, $light, $dark, $white ) = @_;
    my( $poly );
...
sub getColorNFromP
{
    my( $wrap, $darkP, $parts ) = @_;
 
    if ($DEBUG){
      dmsg( "drawColorNFromP()...$parts" );
    }   
...
sub pickColorN
{
    my( $n, $darkP, $blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark  ) = @_;
...
{
  my ($blue, $blueDark, $cyan, $cyanDark, $green, $greenDark, $magenta,  $magentaDark, $red, $redDark ) = @_;
...
sub drawStamp
{
  ...
  my ($img, $realWidth, $realHeight, $grayDark, $black ) = @_;

  my( @date ) = localtime();
  my( $dstr ) = join( "/", $date[4]+1, $date[3], $date[5]+1900 );
  my( $xpos ) = $realWidth - (length( $dstr ) * $fontWidth) - $imgRightBorder;
  my( $ypos ) = $realHeight - $imgBottomBorder + $bottomDataOffset;

  drawString( $img, $dstr, GD::gdSmallFont(), $xpos, $ypos, $grayDark, $black );
}
...
sub countHTML
{
    ...
    print( "<h1>Summary of BLAST Results <A HREF = \"$resultHelp\">Help</A></h1>" );

    print( '<p>' );
    if( $shown < $max )
    {
	print( 'The graph shows the highest hits per range.<br>' );
	print( '<b>Data have been omitted in the Summary Graphic:</b> ' );
