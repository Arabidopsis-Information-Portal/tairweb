...
# STATUS  : Alpha
# REVISION: $Id: Sbjct.pm,v 1.1 2002/03/26 22:56:56 gc Exp $
#
...
#------------------------------------------------------------------------------
#
# [[ this code was broken. it only looked at the first
# HSP (either the author forgot there were possibly other HSPs
# to check, or assumed that the first would always have the
# highest score which is not the case with the BLAST output
# I have, anyway). so far, i've updated
# score and pvalue. - jces ]]
#
#------------------------------------------------------------------------------
...
#----------
    ...
    {
	my( $maxScore ) = -1; # [[ assumes that scores are always non-negative. ]]
	my( $hsp );

	foreach $hsp ( @{$self->{ '_hsps' }} )
	{
	...
	if( $maxScore == -1 )
	{
	    print STDERR "Sbjct.pm: no hsp scores > -1\n";
	    $maxScore = 0;
	}
...
sub pValueSorterIncreasing
{
    ...
    $cmp = ScientificNotation::cmp( $a, $b );

    return( $cmp );
}

sub getMinPValueHelper
{
    my( $sn );

    $sn = $_->p();
    #dmsg( "getMinPValueHelper(): before ", $sn );

    $sn = ScientificNotation::toScientificNotation( $sn );
    #dmsg( "getMinPValueHelper(): after ", $sn );
...
sub getMinPValue
{
    ...
    @pvals = @{$self->{ '_hsps' }};
    @pvals = map( getMinPValueHelper, @pvals );
    @pvals = sort pValueSorterIncreasing @pvals;
    ...
    {
	# bad news, but you didn't give me anything to work with!
	$pval = undef;
    }

    #dmsg( "getMinPValue(): ", $self->name, $pval );
